    ...
    <table style="width: 100%" class="m-table">
        ...
        <tbody>
            ...
            <tr data-toggle="{{$i!=0 ? 'collapse':''}}" data-target="#tr_{{$i}}">
                    ...
                    <td>{{$item->ten}}</td>
                    <td class="w100">{{$item->ten_loai}}</td>
            ...
            <tr id="tr_{{$i}}" class="collapse {{$i==0 ? 'in':''}}">
                ...
                <td colspan="5">
                    ...
                    <p class="text-right">
                        <button v-on:click="isAdd=true;id={{$item->id}};isUpdate=true;getUpdateThuoc()" class="btn btn-success">Cập nhật</button>
